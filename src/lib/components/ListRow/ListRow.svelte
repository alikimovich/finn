...
<script lang="ts">
	...
	interface Props {
		/**
		 * Inner element. `'a'` for navigation, `'button'` for actions,
		 * `'div'` for static rows.
		 * @default 'div'
		 */
		as?: 'div' | 'a' | 'button';
		 ...
		 */
		padding?: 'sm' | 'md';
		/** Link target — required when `as="a"`. */
		href?: string;
		/** Click handler — required when `as="button"`. */
		onclick?: (e: MouseEvent) => void;
		/** Accessible label — describes the row's destination/action. */
		'aria-label'?: string;
		/** Row content; usually a `<Cluster>`. */
		children: Snippet;
	}
