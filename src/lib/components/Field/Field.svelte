...
<script lang="ts">
	...
	interface Props {
		/** Uppercase label text for the form row. */
		label: string;
		/**
		...
		optional?: boolean;
		/**
		 * Wrapping element. Use `'div'` when wrapping a non-form-control
		 * (e.g. `<CurrencyPicker>`); the default `'label'` makes clicking
		 * the label focus its input.
		 * @default 'label'
		 */
		as?: 'label' | 'div';
		/** The control. `<input>`/`<textarea>` are styled automatically. */
		children: Snippet;
	}
