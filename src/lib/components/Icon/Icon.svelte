...
<script lang="ts">
	interface Props {
		/** Glyph name. Add new ones to the `IconName` union and the switch below. */
		name: IconName;
		/**
		 * Icon size, mapped to `--icon-{xs|sm|md|lg}` tokens.
		 * @default 'md'
		 */
		size?: 'xs' | 'sm' | 'md' | 'lg';
		/**
	...
	let { name, size = 'md', strokeWidth = 1.5 }: Props = $props();
...
>
	...
	{:else if name === 'convert'}
		<path d="M3 5h9M9 2l3 3-3 3M13 11H4M7 14l-3-3 3-3" />
	{:else if name === 'edit'}
	...
	{:else if name === 'search'}
		...
		<path d="m11 11 3 3" />
	{:else if name === 'send'}
		<path d="M14 2 7 9M14 2l-5 12-2-5-5-2 12-5Z" />
	{:else if name === 'stories'}
		<rect x="2" y="3" width="12" height="10" rx="2" />
		<path d="M2 7h12" />
	{:else if name === 'swap-vertical'}
